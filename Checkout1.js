...
const Checkout = () => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(false);
    ...
    const [confirmTppPin, setConfirmTppPin] = useState("");

   const handleCreateTppPin = (e) => {
     e.preventDefault(); // Add this line
     if (tppPin === confirmTppPin) {
       // Handle create TPP Pin logic here
       setModalIsOpen(true);
     } else {
       alert("TPP Pin and Confirm TPP Pin do not match");
     }
   };
        ...
        <div className="logo-container">
         ...
         <form>
           <div style={{ display: 'flex', alignItems: 'center' }}>
             <label style={{ marginRight: '10px' }}>Create TPP Pin:</label>
             <input
               type="password"
               value={tppPin}
           ...
           <div style={{ display: 'flex', alignItems: 'center' }}>
                         <label style={{ marginRight: '10px' }}>Confirm TPP Pin:</label>
             <input
               type="password"
               value={confirmTppPin}
               onChange={(e) => setConfirmTppPin(e.target.value)}
               style={{ width: '100px', height: '20px', fontSize: '14px', padding: '5px' }}
             />
             </div>
             <div>
             <Modal
                   ...
                   contentLabel="TPP Pin Created"
                 >
                   <h2>TPP Pin Created</h2>
                   <p>TPP Pin created successfully</p>
                   <button onClick={closeModal}>Close</button>
                 </Modal>
           </div>
           <button onClick={(e) => handleCreateTppPin(e)}>Create TPP Pin</button>
         </form>
        <form>
                <div>
                  <label>         </label>
                  <select value={selectedBank} onChange={handleBankSelect} className="bank-select">
